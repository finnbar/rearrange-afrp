...
toCell :: forall (s :: Nat) t m v c. (Monad m, MonadRW m v, Constr m v t)
    => m (v t) -> m (Cell v s t)
toCell action = action >>= \ptr -> return (Cell @s @t @m ptr)

printCell :: forall v s t. (KnownNat s, MonadRW IO v, Constr IO v t, Show t)
    => Cell v s t -> IO ()
printCell (Cell p) = readVar p >>= \v ->
    putStrLn $ show (natVal (Proxy @s)) ++ ": " ++ show v
...
instance (PrintCells xs, KnownNat s, MonadRW IO v, Constr IO v t, Show t) =>
