...
runMem :: Subset (MemoryUnion s) env =>  MIO  s b -> Set env -> IO b
...
instance (RunMems xs env out, Subset (MemoryUnion s) env) =>
    RunMems ( MIO  s b ': xs) env (b ': out) where
        runMems (mem :+: mems) env = do
            r <- runMem mem env
            rs <- runMems mems env
            return $ r :+: rs

runMem_ :: Subset (MemoryUnion s) env =>  MIO  s () -> Set env -> IO ()
...
instance (CompileMems_ xs env, Subset (MemoryUnion s) env) =>
    CompileMems_ ( MIO  s () ': xs) env where
